...
static UIWindow *HUDActiveWindow(UIApplication *app)
{
    for (UIWindow *window in app.windows.reverseObjectEnumerator) {
        ...
        if ([NSStringFromClass([window.rootViewController class]) isEqualToString:@"HUDRootViewController"]) {
            return window;
        }
    }
...
static __used void _HUDEventCallback(void *target, void *refcon, IOHIDServiceRef service, IOHIDEventRef event)
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        UIWindow *hudWindow = HUDActiveWindow(app);
        if (!hudWindow) {
            return;
        }
        ...
        UIView *keyView = [hudWindow.rootViewController.view hitTest:globalPoint withEvent:nil];
        if (!keyView) {
            keyView = hudWindow.rootViewController.view;
        }

        NSInteger pointerId = [[[[rep handInfo] paths] firstObject] pathIdentity];
        if (pointerId > 0) {
            [TSEventFetcher receiveAXEventID:MIN(MAX(pointerId, 1), 98)
                          atGlobalCoordinate:globalPoint
