...
- (void)loadView
{
    ...
    // Минималистичный чёрный фон: тонкий вертикальный градиент почти-чёрного,
    // без анимаций и цветных переливов.
    ...
    gradientLayer.colors = @[
        (__bridge id)[UIColor colorWithRed:0.05f green:0.06f blue:0.08f alpha:1.0f].CGColor,
        (__bridge id)[UIColor colorWithRed:0.00f green:0.00f blue:0.00f alpha:1.0f].CGColor,
    ];
    ...
    [appIconImageView setTranslatesAutoresizingMaskIntoConstraints:NO];
...
- (void)reloadMainButtonState
{
    ...
    [UIView transitionWithView:self.backgroundView duration:0.25 options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
        [_mainButton setTitle:(_isHUDActive ? NSLocalizedString(@"Stop", nil) : NSLocalizedString(@"Start", nil)) forState:UIControlStateNormal];
...
{
    // No-op: author label doesn't link anywhere.
}
